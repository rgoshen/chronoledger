...
As a **user**, I want to **start an entry and stop it later**, so that I can **track time without knowing the end time
up front**.
...
1. **Given** I start an open entry with a code and start time **When** I save **Then** the entry is created with no end
   time and is clearly marked as “running/open”.
2. **Given** I stop the open entry **When** I submit an end time **Then** the entry becomes complete and is subject to
   locking rules.
3. **Given** I submit duplicate stop requests (retry) **When** the server receives them **Then** the write is
   idempotent and results in a single consistent completed entry.
4. **Given** an open entry would cause overlap **When** I start or stop it **Then** the operation is rejected with a
   clear overlap error.
