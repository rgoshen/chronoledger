...
## Narrative
As a **user**, I want time entries to **remain correct across midnight and time zone changes**, so that pay period totals and exports are trustworthy.
...
1. **Given** I enter time that crosses midnight **When** I save **Then** the system splits it into two entries (one per day) automatically.
2. **Given** my device time zone changes **When** I create or edit entries **Then** the system detects and handles the change according to policy (no silent drift).
3. **Given** entries are stored in UTC **When** I view them **Then** they display in the appropriate local/context time zone per FR-021.
