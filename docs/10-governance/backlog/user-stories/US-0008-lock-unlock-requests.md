...
## Narrative
As a **user**, I want completed entries to **lock automatically** and to **request an unlock when corrections are needed**, so that changes are controlled and auditable.
...
3. **Given** I submit an unlock request **When** an admin reviews it **Then** the admin can approve/reject and the result is recorded in the audit trail.
